<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:45:"F:\www\hiv/app/admin\view\auth\adminRule.html";i:1519691410;s:42:"F:\www\hiv/app/admin\view\common\head.html";i:1519691410;s:42:"F:\www\hiv/app/admin\view\common\foot.html";i:1519691410;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="admin-main layui-anim layui-anim-upbit">
    ...
    <blockquote class="layui-elem-quote">
        ...
        <buttion class="layui-btn layui-btn-sm" id="isCanSeeAll">批量设置配置</buttton>
    </blockquote>
...
<script type="text/html" id="is_can_see">
    <input type="checkbox" name="is_can_see" value="{{d.id}}" lay-skin="switch" lay-text="可配|不可" lay-filter="is_can_see" {{ d.is_can_see == 1 ? 'checked' : '' }}>
</script>
    ...
    layui.use(['table','form'], function() {
        ...
        tableIn = table.render({
            ...
            cols: [[
                ...
                {field: 'is_can_see',align: 'center',title: '配置情况', width: 150,toolbar: '#is_can_see'},
        ...
        form.on('switch(is_can_see)', function(obj){
            loading =layer.load(1, {shade: [0.1,'#fff']});
            var id = this.value;
            var is_can_see = obj.elem.checked===true?1:0;
            $.post('<?php echo url("isCanSee"); ?>',{'id':id,'is_can_see':is_can_see},function (res) {
                layer.close(loading);
                if (res.status==1) {
                    tableIn.reload();
                }else{
        ...
        $('#isCanSeeAll').click(function(){
            var checkStatus = table.checkStatus('adminRule'); //test即为参数id设定的值
            var ids = [];
            ...
            var is_can_see;
            layer.confirm('确认要批量设置可配置状态吗？', {
                btn: ['不可配置','可配置'] //按钮
            }, function(){
                is_can_see=0;
                $.post("<?php echo url('isCanSeeAll'); ?>", {ids: ids,is_can_see:is_can_see}, function (data) {
                    layer.close(loading);
                    if (data.code === 1) {
